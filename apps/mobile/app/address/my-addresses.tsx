...
const MyAddressesScreen = () => {
  ...
  const handleDelete = (id: string, isDefault: boolean) => {
    if (isDefault) {
      Alert.alert(
        "Không thể xóa",
        "Không thể xóa địa chỉ mặc định. Hãy chỉnh sửa và đặt địa chỉ khác làm mặc định trước.",
      );
      return;
    }
    ...
    <SafeAreaView style={{ flex: 1 }} className="bg-zinc-50 dark:bg-zinc-950">
                  ...
                  >
                    ...
                    <HStack className="justify-between items-start mb-3">
                      <HStack className="items-center space-x-2 gap-2">
                        <Box
                          className={`w-8 h-8 rounded-full items-center justify-center ${cfg.bg}`}
                        >
                          <Icon
                            as={cfg.icon}
                            className={`${cfg.color} w-4 h-4`}
                            size="xs"
                          />
                        </Box>
                        ...
                        {address.isDefault && (
                          <Box className="bg-yellow-500 px-2 py-0.5 rounded-md">
                            <Text className="text-[10px] font-bold text-zinc-900 uppercase">
                              Mặc định
                            </Text>
                          </Box>
                        )}
                      ...
                      <HStack className="space-x-3 gap-3">
                        ...
                        <Pressable
                          onPress={() =>
                            handleDelete(address._id, address.isDefault)
                          }
                          className="p-1"
                        >
                    ...
                    <VStack className="space-y-1 gap-1 mb-3">
                      ...
                      <HStack className="items-start space-x-2 gap-2 mt-1">
                        <Icon
                          as={MapPin}
                          className="text-yellow-500 w-4 h-4 mt-0.5"
                        />
                    ...
                    {address.isDefault && (
                      <HStack className="items-center space-x-2 gap-2 pt-3 border-t border-zinc-50 dark:border-zinc-800">
                        ...
                        <Text className="text-xs font-bold text-yellow-600">
                          Đang dùng làm địa chỉ mặc định
                        </Text>
                      </HStack>
                    )}
                  </Box>
      ...
      <Box className="p-5 bg-white dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800">
        ...
        >
          <ButtonIcon
            as={Plus}
            className="text-white dark:text-zinc-900 mr-2"
          />
