      ...
      <Box className="bg-white dark:bg-zinc-950 px-5 pt-2 pb-4 border-b border-zinc-100 dark:border-zinc-900 shadow-sm">
        <HStack className="items-center space-x-3 gap-3">
          ...
          <Pressable onPress={() => router.back()}>
            <Text className="text-sm font-black text-yellow-600 dark:text-yellow-500 uppercase tracking-tighter">
              Hủy
            </Text>
          </Pressable>
        </HStack>
            ...
            <VStack className="space-y-4 gap-4">
              <HStack className="justify-between items-center">
                <HStack className="items-center space-x-2 gap-2">
                  ...
                  <Text className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-widest">
                    Tìm kiếm gần đây
                  </Text>
                </HStack>
                <Pressable>
                  <Icon as={Trash2} size="sm" className="text-zinc-300" />
                </Pressable>
              </HStack>
              ...
              <HStack className="justify-between">
                {suggestedCategories.map((item) => (
                  <Pressable
                    ...
                    onPress={() =>
                      router.push(
                        `/category/productFollCate?name=${item.name}` as any,
                      )
                    }
                    className="items-center space-y-2 gap-2"
                  >
                    <Box
                      className={`w-16 h-16 rounded-3xl items-center justify-center shadow-sm ${item.color}`}
                    >
                      <Icon
                        as={item.icon}
                    ...
                    <Text className="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">
                      {item.name}
                    </Text>
                  </Pressable>
                ))}
                ...
                >
                  <HStack className="space-x-4 gap-4">
                    <Image
                      ...
                      className="w-16 h-16 rounded-2xl bg-zinc-50 dark:bg-zinc-800"
                    />
                    <VStack className="flex-1 justify-center">
                      ...
                      <Text className="text-sm font-black text-yellow-600 mt-1">
                        {formatPrice(item.basePrice)}₫
                      </Text>
                    </VStack>
                    <Icon
                      as={ChevronRight}
                      className="text-zinc-200 self-center"
                    />
                  </HStack>
                </Pressable>
              ))}
          ...
          <VStack className="p-5 space-y-6 gap-6">
            ...
            <HStack className="justify-between items-center">
              ...
              <Pressable className="flex-row items-center space-x-2 gap-2 bg-zinc-900 dark:bg-yellow-500 px-4 py-2 rounded-xl">
                ...
                />
                <Text className="text-[10px] font-black text-white dark:text-zinc-900 uppercase">
                  Lọc
                </Text>
              </Pressable>
            </HStack>
