...
const mockReviews = [
  {
    ...
    date: "2024-03-15",
    comment:
      "Sản phẩm chất lượng, đóng gói cẩn thận. Rất đáng đồng tiền bát gạo!",
  },
  {
    id: "r2",
    user: "Trần Thị B",
    ...
    <SafeAreaView style={{ flex: 1 }} className="bg-zinc-50 dark:bg-zinc-950">
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
                  ...
                  {formatPrice(
                    matchingVariant
                      ? matchingVariant.price *
                          (1 - (matchingVariant.discountPercentage || 0) / 100)
                      : product.finalPrice ||
                          product.basePrice *
                  ...
                  <HStack className="flex-wrap gap-3">
                    {attrValues.map((val) => {
                      ...
                      return (
                        ...
                        >
                          <Text
                            className={`text-xs font-bold ${isSelected ? "text-zinc-900 dark:text-yellow-500" : "text-zinc-500"}`}
                          >
                            {val}
                          </Text>
                        </Pressable>
          ...
          <Box className="px-5 py-4 bg-zinc-50 dark:bg-zinc-950 flex-row justify-between border-y border-zinc-100 dark:border-zinc-900">
            <VStack className="items-center flex-1">
              ...
              <Text className="text-[10px] text-zinc-500 text-center">
                Free Delivery
              </Text>
            </VStack>
            ...
            Object.keys(product.specifications).length > 0 && (
              <Box className="px-5 py-6 bg-white dark:bg-zinc-900 mt-2">
                ...
                {Object.entries(product.specifications).map(([key, value]) => (
                  <HStack
                    key={key}
                    className="py-3 border-b border-zinc-100 dark:border-zinc-800 justify-between"
                  >
                    <Text className="text-sm text-zinc-500 flex-1 pr-4">
                      {key}
                    </Text>
                    <Text className="text-sm font-semibold text-zinc-900 dark:text-white flex-1 text-right">
                      {value as string}
                    </Text>
                  </HStack>
                ))}
          ...
          <Box className="px-5 py-6 bg-white dark:bg-zinc-900 mt-2">
            <HStack className="justify-between items-center mb-6">
              <VStack>
                ...
                <Text className="text-xs text-zinc-500">
                  Dựa trên {product.reviewCount || 0} nhận xét
                </Text>
              </VStack>
              <Pressable>
      ...
      <Box className="absolute bottom-0 left-0 right-0 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 px-5 py-4 pb-8">
        <HStack className="space-x-4 gap-4">
          ...
          >
            <ButtonText className="text-zinc-900 font-extrabold text-base uppercase">
              Mua ngay
            </ButtonText>
          </Button>
        </HStack>
                  ...
                  <VStack className="flex-1 justify-center">
                      ...
                      <Text className="text-2xl font-extrabold text-zinc-900 dark:text-white">
                        {formatPrice(
                          matchingVariant
                            ? matchingVariant.price *
                                (1 -
                                  (matchingVariant.discountPercentage || 0) /
                                    100)
                            : product.finalPrice ||
                                product.basePrice *
                                  (1 - (product.discountPercentage || 0) / 100),
                    ...
                    {(matchingVariant?.discountPercentage ||
                      ...
                      <Text className="text-sm text-zinc-400 line-through mb-1">
                        {formatPrice(
                          matchingVariant?.price || product.basePrice,
                        )}
                        ₫
                      </Text>
                    )}
                ...
                >
                  {Object.keys(variantOptions).length > 0 &&
                    Object.entries(variantOptions).map(
                      ([attrName, attrValues]) => (
                        <Box key={attrName} className="mb-4">
                          <Text className="text-sm font-bold text-zinc-900 dark:text-white mb-3">
                            {attrName}
                          </Text>
                  ...
                  <HStack className="justify-between items-center mb-10 mt-4">
                    ...
                    <HStack className="items-center bg-zinc-100 dark:bg-zinc-800 rounded-2xl p-1 gap-4">
                      ...
                      >
                        <Icon
                          as={Minus}
                          className="text-zinc-900 dark:text-white w-4 h-4"
                        />
                      </Pressable>
                      ...
                      <Pressable
                        onPress={() => {
                          ...
                          if (quantity < maxStock) {
                            ...
                            Alert.alert(
                              "Thông báo",
                              "Số lượng vượt quá số lượng trong kho.",
                            );
                          }
                        }}
