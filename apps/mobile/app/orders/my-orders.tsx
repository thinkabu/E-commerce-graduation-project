          ...
          {mockOrders.map((order) => {
            ...
            return (
              <Pressable
                key={order.id}
                onPress={() =>
                  router.push(`/orders/order-detail?id=${order.id}` as any)
                }
              ...
              >
                <HStack className="justify-between items-center mb-4">
                  ...
                  <Box
                    className={`${status.bg} px-3 py-1.5 rounded-xl flex-row items-center`}
                  >
                    <Icon as={status.icon} size="xs" className={status.color} />
                    <Text
                      className={`text-[10px] font-black ml-1.5 uppercase ${status.color}`}
                    >
                      {status.label}
                    </Text>
                  </Box>
