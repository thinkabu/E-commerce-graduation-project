...
import { ScrollView, SafeAreaView, Dimensions, Image, FlatList, ActivityIndicator } from 'react-native';
...
const CategoryProductsScreen = () => {
  ...
  const { name = 'Danh mục', id } = useLocalSearchParams<{ name: string, id: string }>();
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [products, setProducts] = useState<Product[]>([]);
  const [categories, setCategories] = useState<Category[]>([]);
  const [loading, setLoading] = useState(true);
  
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | undefined>(id === 'all' || !id ? undefined : id);
    ...
    <SafeAreaView style={{ flex: 1 }} className="bg-zinc-50 dark:bg-zinc-950">
      ...
      <VStack className="flex-1">
            ...
            >
              ...
              >
                <Text className={`text-xs font-bold ${
                  !selectedCategoryId ? 'text-zinc-900' : 'text-zinc-600 dark:text-zinc-400'
                }`}>
                  Tất cả danh mục
                </Text>
              </Pressable>
              ...
              {categories.map((cat) => (
                ...
                >
                  ...
                  <Text className={`text-xs font-bold ${
                    selectedCategoryId === cat._id ? 'text-zinc-900' : 'text-zinc-600 dark:text-zinc-400'
                  }`}>
                    {cat.name}
                  </Text>
                </Pressable>
        ...
        <HStack className="px-5 py-4 justify-between items-center">
          <Text className="text-sm font-bold text-zinc-500">{products.length} sản phẩm</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="flex-1 px-5">
          {loading ? (
                ...
                {products.map((product) => (
                  ...
                  >
                    <HStack className="space-x-4 gap-4">
                      <Image source={{ uri: product.images?.[0] || 'https://via.placeholder.com/300' }} className="w-24 h-24 rounded-2xl bg-zinc-50 dark:bg-zinc-800" />
                      <VStack className="flex-1 justify-center space-y-1 gap-1">
                        ...
                        <Text className="text-base font-black text-yellow-600 dark:text-yellow-500">{formatPrice(product.basePrice)}₫</Text>
                      </VStack>
                    </HStack>
                  </Pressable>
                ))}
            ...
            <VStack className="flex-1 justify-center items-center py-20">
              <Text className="text-zinc-400 font-medium">Không có sản phẩm nào trong danh mục này</Text>
            </VStack>
          )}
        </ScrollView>
      </VStack>
