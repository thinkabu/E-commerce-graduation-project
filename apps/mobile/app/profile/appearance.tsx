      ...
      >
        ...
        <VStack className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
          {themeOptions.map((option, index) => {
            ...
            return (
              ...
              >
                <Box
                  className={`w-12 h-12 rounded-2xl ${option.iconBg} items-center justify-center mr-4`}
                >
                  <Icon
                    as={option.icon}
                    className={`${option.iconColor} w-6 h-6`}
                  />
                </Box>
        ...
        <Box className="mt-6 px-3">
          <Text className="text-xs text-zinc-400 leading-relaxed">
            Chế độ "Theo hệ thống" sẽ tự động chuyển đổi giữa giao diện sáng và
            tối dựa trên cài đặt trên thiết bị của bạn.
          </Text>
        </Box>
      </ScrollView>
    </SafeAreaView>
