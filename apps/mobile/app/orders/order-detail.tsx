    ...
    items: [
      {
        ...
        quantity: 1,
        image:
          "https://images.unsplash.com/photo-1695048133142-1a20484d2569?q=80&w=200&auto=format&fit=crop",
      },
      {
        id: "2",
        name: "Ốp lưng Silicone Case with MagSafe",
        price: 1450000,
        quantity: 1,
        image:
          "https://images.unsplash.com/photo-1603302576837-37561b2e2302?q=80&w=200&auto=format&fit=crop",
      },
    ],
    address: {
      name: "Nguyễn Văn A",
    ...
    <SafeAreaView style={{ flex: 1 }} className="bg-zinc-50 dark:bg-zinc-950">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        ...
        <Box className="bg-yellow-500 p-8 rounded-b-[40px] items-center justify-center">
          ...
          <Text className="text-2xl font-black text-zinc-900 uppercase tracking-tighter">
            Đang xử lý
          </Text>
          <Text className="text-sm text-zinc-900/60 font-medium mt-1">
            Dự kiến giao hàng: 05/05/2026
          </Text>
        </Box>
        ...
        <VStack className="px-5 -mt-6 space-y-6 gap-6 pb-20">
          ...
          <Box className="bg-white dark:bg-zinc-900 p-5 rounded-3xl shadow-sm border border-zinc-100 dark:border-zinc-800">
            <HStack className="justify-between items-center pb-4 border-b border-zinc-50 dark:border-zinc-800 mb-4">
              <VStack>
                <Text className="text-[10px] text-zinc-400 font-black uppercase tracking-widest">
                  Mã đơn hàng
                </Text>
                <HStack className="items-center space-x-2 gap-2">
                  <Text className="text-base font-black text-zinc-900 dark:text-white uppercase">
                    #{mockOrder.id}
                  </Text>
                  <Pressable>
                    <Icon as={Copy} size="xs" className="text-zinc-300" />
                  </Pressable>
                </HStack>
              </VStack>
              ...
              {mockOrder.timeline.map((step, index) => (
                <HStack key={index} className="items-start space-x-3 gap-3">
                  <VStack className="items-center">
                    <Box
                      className={`w-5 h-5 rounded-full items-center justify-center ${step.completed ? "bg-yellow-500" : "bg-zinc-100 dark:bg-zinc-800"}`}
                    >
                      <Box
                        className={`w-2 h-2 rounded-full ${step.completed ? "bg-zinc-900" : "bg-zinc-300"}`}
                      />
                    </Box>
                    {index < mockOrder.timeline.length - 1 && (
                      <Box
                        className={`w-0.5 h-6 ${step.completed ? "bg-yellow-500" : "bg-zinc-100 dark:bg-zinc-800"}`}
                      />
                    )}
                  </VStack>
                  <VStack className="flex-1 -mt-0.5">
                    <Text
                      className={`text-sm font-bold ${step.completed ? "text-zinc-900 dark:text-white" : "text-zinc-400"}`}
                    >
                      {step.title}
                    </Text>
                    {step.time !== "--" && (
          ...
          <VStack className="space-y-3 gap-3">
            ...
            <Box className="bg-white dark:bg-zinc-900 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800">
              <HStack className="items-center space-x-3 gap-3 mb-3">
                ...
                <VStack>
                  <Text className="text-base font-black text-zinc-900 dark:text-white">
                    {mockOrder.address.name}
                  </Text>
                  <Text className="text-sm text-zinc-500 font-medium">
                    {mockOrder.address.phone}
                  </Text>
                </VStack>
              </HStack>
          ...
          <VStack className="space-y-3 gap-3">
            <Text className="text-xs font-black text-zinc-400 uppercase tracking-widest ml-1">
              Sản phẩm ({mockOrder.items.length})
            </Text>
            {mockOrder.items.map((item) => (
              <Box
                key={item.id}
                className="bg-white dark:bg-zinc-900 p-4 rounded-3xl border border-zinc-100 dark:border-zinc-800"
              >
                <HStack className="space-x-4 gap-4">
                  <Image
                    source={{ uri: item.image }}
                    className="w-20 h-20 rounded-2xl bg-zinc-50"
                  />
              ...
              <VStack className="space-y-3 gap-3">
                ...
                <HStack className="justify-between items-center">
                  ...
                  <Text className="text-yellow-500 text-xl font-black">
                    {formatPrice(mockOrder.payment.total)}₫
                  </Text>
                </HStack>
              </VStack>
            </Box>
